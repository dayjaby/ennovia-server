...
s32 CGUITTFont::getWidthFromCharacter(wchar_t c) const
{
	u32 n = const_cast<CGUITTFont*>(this)->getGlyphByChar(c);
	if ( n > 0){
		int w = const_cast<CGUITTFont*>(this)->Glyphs[n - 1].texw;
		s32 left = const_cast<CGUITTFont*>(this)->Glyphs[n - 1].left;
		if (w + left > 0) return w + left;
	}
	if (c >= 0x2000){
		return	const_cast<CGUITTFont*>(this)->Glyphs[0].size;
	} else {
		return	const_cast<CGUITTFont*>(this)->Glyphs[0].size / 2;
	}
}
